import UIKit

//함수 function
//함수의 용도
//1. 코드를 기능별로 분리하고, 복잡한 코드를 간결하게 함
//2. 코드블럭에 이름을 붙일 수 있음
//3. 코드를 재사용할 수 있음 - 코드의 중복을 피할 수 있음

//func 함수이름( 매개변수 이름: 타입 ) -> 반환형타입 {
//  수행문...
//}

func myFunc(){
    print("call myFunc")
}
myFunc();

//함수의 4가지 패턴
//1. 매개변수 없고, 반환형 없음
func myFunc1() -> Void/*생략가능*/{
    print("call")
}
myFunc1()
//2. 매개변수 있고, 반황형 없음
func myFunc2(name: String){
    print("call 2 "+name)
}
myFunc2(name: "apple")//매개변수 이름과 값을 동시에 넣어줌
//3. 매개변수 없고, 반환형 있고
func myFunc3() -> Int{
    print("call3")
    return 10
}
//4. 매개변수 있고, 반환형 있음
func myFunc4(name: String) -> String{
    print("call")
    return name + "return"
}
myFunc4(name: "banana")

//함수의 고급 문법

//매개변수를 여러개 넣을 때
func printMyName(name1: String, name2: String){
    print("name1 \(name1) name2 \(name2)")
}
printMyName(name1: "a", name2: "b")

//매개변수의 기본값을 넣을 수 있음
func print2(name1: String, name2: String = "a"){
    print("\(name1) \(name2)")
}
print2(name1: "aa")
print2(name1: "1", name2: "2")//둘 다 가능

//호출시 매개변수 이름 생략 가능
func print3(_ name1: String, _ name2: String){//언더바 사이의 공간 필수
    print(name1, name2, "a")
}
print3("1", "2")

//매개변수 이름에 별명(레이블)을 줄 수 있음
func print4(hero name1: String, actor name2: String){
    print(name1, name2)
}
print4(hero: "a", actor: "b")

//가변 매개변수 - 매개변수를 매열로 입력할 수 있음
func print5(name1: String, name2: String...){
    print(name1, name2)
    for name in name2{
        print(name)
    }
}
print5(name1: "홍길동", name2: "변사또", "춘향이", "이몽룡")
